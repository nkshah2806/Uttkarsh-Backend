...
const { translateTextsDetailed } = require("./translationService");

/**
 * Batch-translate a list of English strings into `target` and return a
 * `Map<source, translation>`.
 *
 * "Added parameter content" is authored in English only (raw_content_en ->
 * parsed_nodes_en). When no pre-authored Hindi/Gujarati variant exists the
 * report previously fell back to English; this helper supplies a machine
 * translation so the generated report honours the selected language.
 *
 * Translation failures are swallowed (the caller falls back to the English
 * source) so report generation is never blocked by a translation outage.
 */
const buildTranslationMap = async (strings, target) => {
  const unique = [...new Set((strings || []).filter((s) => typeof s === "string" && s.trim()))];
  if (unique.length === 0) return new Map();
  try {
    const results = await translateTextsDetailed(unique, target, "en");
    const map = new Map();
    unique.forEach((src, idx) => {
      const r = results[idx];
      if (r && r.ok && r.text && r.text !== src) map.set(src, r.text);
    });
    return map;
  } catch (err) {
    console.error(`[PDF Report] translation batch failed for ${target}:`, err?.message || err);
    return new Map();
  }
};
...
exports.generateReportHTML = async (visitId, lang = "en", options = {}) => {
  ...
  if (activeDisclaimerDoc) {
    disclaimer = {
      ...
      content_gu: activeDisclaimerDoc.content_gu,
    };
  ...
  // Resolve a label/value for the active language with English fallback.
  const L = (en, hi, gu) => (isHindi ? hi || en : isGujarati ? gu || hi || en : en);

  const labels = {
    title: L(
      "QUANTUM RESONANCE WELLNESS ASSESSMENT REPORT",
      "क्वांटम रेज़ोनेंस वेलनेस मूल्यांकन रिपोर्ट",
      "ક્વોન્ટમ રેઝોનન્સ વેલનેસ મૂલ્યાંકન રિપોર્ટ"
    ),
    patientInfo: L(
      "Patient Demographics & Vitals",
      "रोगी जनसांख्यिकी एवं विवरण",
      "દર્દીની માહિતી અને માપ"
    ),
    patientCode: L("Client ID", "क्लाइंट आईडी / कोड", "ક્લાયન્ટ આઈડી / કોડ"),
    name: L("Full Name", "रोगी का नाम", "દર્દીનું નામ"),
    ageGender: L("Age / Gender", "आयु / लिंग", "ઉંમર / લિંગ"),
    mobile: L("Mobile Phone", "मोबाइल नंबर", "મોબાઈલ નંબર"),
    weightHeight: L("Weight / Height", "वजन / ऊंचाई", "વજન / ઊંચાઈ"),
    address: L("Address", "पता", "સરનામું"),
    date: L("Scan Date", "जांच दिनांक", "તપાસ તારીખ"),
    consultant: L("Wellness Consultant", "वेलनेस परामर्शदाता", "વેલનેસ સલાહકાર"),
    summary: L(
      "Wellness Parameters Requiring Attention",
      "ध्यान देने योग्य वेलनेस मापदंड",
      "ધ્યાન આપવા લાયક વેલનેસ પરિમાણો"
    ),
    parameter: L("Parameter Evaluated", "मापदंड विवरण", "પરિમાણ વિગતો"),
    value: L("Observed Value", "प्राप्त मान", "પ્રાપ્ત મૂલ્ય"),
    range: L("Reference Range*", "संदर्भ सीमा*", "સંદર્ભ શ્રેણી*"),
    status: L("Assessment Status", "मूल्यांकन स्थिति", "મૂલ્યાંકન સ્થિતિ"),
    detailedAnalysis: L(
      "Selected Wellness Information & Ayurvedic Lifestyle Guidance",
      "चयनित वेलनेस सूचना एवं आयुर्वेदिक जीवनशैली मार्गदर्शन",
      "પસંદ કરેલ વેલનેસ માહિતી અને આયુર્વેદિક જીવનશૈલી માર્ગદર્શન"
    ),
    nextVisitTitle: L(
      "Suggested Wellness Reassessment Date",
      "सुझाई गई वेलनेस पुनर्मूल्यांकन तिथि",
      "સૂચવેલ વેલનેસ પુનર્મૂલ્યાંકન તારીખ"
    ),
    nextVisitLabel: L(
      "Suggested Wellness Reassessment Date",
      "सुझाई गई वेलनेस पुनर्मूल्यांकन तिथि",
    ...
    medicinesTitle: L(
      "Recommended Medicines & Wellness Support",
      "अनुशंसित औषधियाँ एवं वेलनेस सहायता",
      "ભલામણ કરેલ દવાઓ અને વેલનેસ સહાય"
    ),
    medicinesSubtitle: L(
      "Selected Medicines & Usage Guidance",
      "चयनित औषधियाँ एवं सेवन मार्गदर्शन",
      "પસંદ કરેલ દવાઓ અને ઉપયોગ માર્ગદર્શન"
    ),
    medicineName: L("Medicine", "औषधि का नाम", "દવાનું નામ"),
    medicineDetails: L("Details", "विवरण", "વિગતો"),
    medicineDosage: L("Dosage / Usage", "मात्रा / सेवन विधि", "માત્રા / ઉપયોગ"),
    medicineNoteLabel: L("Consultant Note", "परामर्शदाता टिप्पणी", "સલાહકારની નોંધ"),
    disclaimerHeading: L("Disclaimer", "अस्वीकरण (Disclaimer)", "અસ્વીકરણ (Disclaimer)"),
    footerNote: L(
      ...
      "ઉત્કર્ષ ક્વોન્ટમ વેલનેસ સિસ્ટમ દ્વારા તૈયાર કરાયેલ ગોપનીય વેલનેસ મૂલ્યાંકન રિપોર્ટ."
    ),
    page: L("Page", "पृष्ठ", "પૃષ્ઠ"),
  };
  ...
  //
  // Guidance content authored through the Smart Content Editor lives only in
  // English. Collect every selected parameter name / section title / bullet /
  // disclaimer that has NO pre-authored variant for the active language and
  // machine-translate it, so the report never silently shows English.
  //
  // A localized value that is missing OR identical to the English source is
  // treated as "not translated" — otherwise legacy rows whose *_hi/*_gu were
  // backfilled with the English text would suppress machine translation.
  // ----------------------------------------------------------------------
  const hasLocalized = (localized, source) => {
    const value = typeof localized === "string" ? localized.trim() : "";
    return Boolean(value) && value !== (source || "").trim();
  };

  let contentTranslations = new Map();
  if (isHindi || isGujarati) {
    const stringsToTranslate = [];
    filteredParameters.forEach((item) => {
      const p = item.parameter || {};
      if (!hasLocalized(isHindi ? p.name_hi : p.name_gu, p.name_en)) {
        stringsToTranslate.push(p.name_en);
      }
      (item.sections || []).forEach((sec) => {
        if (!hasLocalized(isHindi ? sec.title_hi : sec.title_gu, sec.title_en)) {
          stringsToTranslate.push(sec.title_en);
        }
        (sec.items || []).forEach((bullet) => {
          if (!hasLocalized(isHindi ? bullet.text_hi : bullet.text_gu, bullet.text_en)) {
            stringsToTranslate.push(bullet.text_en);
          }
        });
      });
    });
    if (!hasLocalized(isHindi ? disclaimer.content_hi : disclaimer.content_gu, disclaimer.content)) {
      stringsToTranslate.push(disclaimer.content);
    }
    contentTranslations = await buildTranslationMap(stringsToTranslate, lang);
  }

  // Resolve a content string for the active language, preferring the
  // pre-authored variant, then the machine translation, then English.
  const T = (en, hi, gu) => {
    if (isHindi) {
      return hasLocalized(hi, en) ? hi : contentTranslations.get(en) || en || "";
    }
    if (isGujarati) {
      return hasLocalized(gu, en) ? gu : contentTranslations.get(en) || en || "";
    }
    return en || "";
  };

  // Format disclaimer paragraphs — honour the selected language, falling back
  // to the English content (or its machine translation) when no localized
  // variant exists.
  const rawDisclaimerText = T(
    disclaimer.content || "",
    disclaimer.content_hi,
    disclaimer.content_gu
  );
...
<head>
  ...
  <style>
    /* Unicode fonts for Devanagari (Hindi) and Gujarati scripts so the printed /
       PDF report renders correctly in every supported language. */
    ...
    * {
    ...
    body {
      font-family: ${isHindi || isGujarati
      ? "'Noto Sans Devanagari', 'Noto Sans Gujarati', 'Nirmala UI', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif"
  ...
  <table class="summary-table">
    ...
    <tbody>
      ...
      : abnormalResults
        ...
        <tr>
          <td>
            <strong>${escapeHTML(T(r.parameter.name_en, r.parameter.name_hi, r.parameter.name_gu))}</strong>
          </td>
  ...
  ${filteredParameters
      ...
      <div class="param-block">
        <div class="param-header">
          <div>
            <span class="param-name">${escapeHTML(T(p.name_en, p.name_hi, p.name_gu))}</span>
          ...
          ${item.sections
            .map((sec) => `
              <div class="guidance-section-title">
                <span>▸</span> ${escapeHTML(T(sec.title_en, sec.title_hi, sec.title_gu))}
              </div>
              <ul class="bullet-list">
                ...
                .map(
                  ...
                  <li style="${bullet.level > 1 ? 'margin-left: 14px; list-style-type: circle;' : ''}">
                    ${escapeHTML(T(bullet.text_en, bullet.text_hi, bullet.text_gu))}
                  </li>
                `
                )
                .join("")}
